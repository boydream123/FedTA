...
#         color_transform = transforms.Compose([get_color_distortion(), PILRandomGaussianBlur()])
        if cifar_mean:
            ...
            logger.info('cifar mean')
        else:
            # imageNet mean
            mean = [0.485, 0.456, 0.406]
            std = [0.228, 0.224, 0.225]
            logger.info('ImageNet mean')
        for i in range(len(size_crops)):
            if multi_crop:
                logger.info('multiCrop enabled')
                randomresizedcrop = transforms.RandomResizedCrop(
                    size_crops[i],
                    # 貌似这里是对图片进行不同大小的截取,开启
                    scale=(min_scale_crops[i], max_scale_crops[i]),
            )
            else:
                logger.info('multiCrop disabled')
                randomresizedcrop = transforms.RandomResizedCrop(size_crops[i])
